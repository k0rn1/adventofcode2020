import sequtils, strutils, typetraits, json, math, sequtils, strutils, strutils, algorithm
...
var seatIds = entries.map(proc(x: Seat): int = x.seatId)
seatIds.sort()


var last = seatIds[0]
for idx, id in seatIds.pairs:
    if idx != 0 and (last + 1) != id:
      echo "Seat ", id - 1, " is missing"
    last = id
